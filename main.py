...
class BotUI(ctk.CTk):
    ...
    def on_auto_trade_toggle(self):
        if self.var_auto_trade.get():
            ...
            self.log_message("🔴 AUTO-TRADE DAEMON ĐÃ TẮT. Chuyển về chế độ bắn tay (Manual).")
    ...
    def bg_update_loop(self):
        while self.running:
            try:
                sym = self.cbo_symbol.get()
                new_map = self.trade_mgr.update_running_trades(self.cbo_account_type.get(), self.latest_market_context)
    ...
    def update_ui(self, acc, state, check_res, tick, preset, sym, positions):
        ...
        pnl = state['pnl_today']
        self.lbl_stats.configure(text=f"Ngày hôm nay: ${pnl:.2f}", text_color=COL_GREEN if pnl >= 0 else COL_RED)
        ...
        if tick and acc:
            ...
            current_risk_pct = brain.get("risk_tsl", {}).get("base_risk", params.get("RISK_PERCENT", 0.3))
            ...
            if f_lot < config.MIN_LOT_SIZE:
                ...
                self.lbl_prev_lot.configure(text=f"{'(TAY)' if mlot>0 else '(TỰ ĐỘNG)'} VOL: {f_lot:.2f}", text_color="white" if mlot==0 else "#FFD700")
            ...
            acc_type = self.cbo_account_type.get()
            if acc_type in ["PRO", "STANDARD"]: self.lbl_fee_info.configure(text=f"Chi phí (Spread): -${spread_cost:.2f}")
            else: self.lbl_fee_info.configure(text=f"Chi phí (Comm): -${comm_total:.2f}")
    ...
    def on_click_trade(self):
        ...
        def run_trade_thread():
            ...
            if "SUCCESS" not in result: self.log_message(f"❌ THẤT BẠI: {result}", error=True)
    ...
    def close_all_trades(self):
        ...
        if not items: return
        if self.var_confirm_close.get() and not messagebox.askyesno("Xác nhận", "ĐÓNG TOÀN BỘ LỆNH?"): return
        for item in items:
            p = next((p for p in self.connector.get_all_open_positions() if p.ticket==int(item)), None)
            if p: threading.Thread(target=lambda: self.connector.close_position(p)).start()

    def close_selected_trades(self):
        selected = self.tree.selection()
        if not selected: return
        if self.var_confirm_close.get() and not messagebox.askyesno("Xác nhận", f"Đóng {len(selected)} lệnh đã chọn?"): return
    ...
    def on_tree_click(self, event):
        ...
        if region == "cell":
            ...
            if row_id and col == "#9":
                if self.var_confirm_close.get() and not messagebox.askyesno("Xác nhận", f"Đóng lệnh #{row_id}?"): return
                p = next((p for p in self.connector.get_all_open_positions() if p.ticket==int(row_id)), None)
                if p: threading.Thread(target=lambda: self.connector.close_position(p)).start()
    ...
    def on_tree_right_click(self, event):
        ...
        if len(selected) > 1:
            menu.add_command(label=f"❌ Đóng {len(selected)} Lệnh Đã Chọn", command=self.close_selected_trades)
        else:
            if row_id:
                self.tree.selection_set(row_id)
                ticket = int(row_id)
                menu.add_command(label=f"📝 Sửa lệnh #{ticket}", command=lambda: self.open_edit_popup(ticket))
                menu.add_separator()
                menu.add_command(label="❌ Đóng Lệnh Này", command=lambda: self.close_selected_trades())
