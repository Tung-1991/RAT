...
class ExnessConnector:
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            if not account_info:
                ...
                return False
            logger.info(
                f"Đã kết nối thành công tới tài khoản #{account_info.login} trên server {account_info.server}"
            )
    ...
    def get_historical_data(
        ...
        try:
            ...
            if rates is None or len(rates) == 0:
                logger.warning(
                    f"Không có dữ liệu lịch sử cho {symbol} trên khung {timeframe}."
                )
                return None
            df = pd.DataFrame(rates)
            df["time"] = pd.to_datetime(df["time"], unit="s")
            df.rename(
                columns={"time": "timestamp", "tick_volume": "volume"}, inplace=True
            )
            df.set_index("timestamp", inplace=True)
            return df[["open", "high", "low", "close", "volume"]]
        except Exception as e:
            logger.error(
                f"Lỗi ngoại lệ khi lấy dữ liệu lịch sử cho {symbol}: {e}", exc_info=True
            )
            return None
    ...
    def place_order(
        ...
        is_valid, reason = self.validate_order_before_placement(
        ...
        if result and result.retcode == mt5.TRADE_RETCODE_DONE:
            logger.info(
                f"✅ Lệnh {symbol} đã được đặt thành công. Ticket: {result.order}, Comment: '{comment}'"
            )
            return result
        logger.error(
            f"❌ Đặt lệnh {symbol} thất bại. Retcode: {result.retcode if result else 'N/A'}, Comment: '{result.comment if result else 'N/A'}' Error: {mt5.last_error()}"
        )
        return None
    ...
    def close_position(
        self,
        position,
        volume_to_close: Optional[float] = None,
        comment: str = "exness_bot_close",
    ) -> Optional[mt5.TradeResult]:
        if not self._is_connected:
            return None
        ...
        if result and result.retcode == mt5.TRADE_RETCODE_DONE:
            logger.info(
                f"✅ Lệnh đóng {volume:.2f} lot cho ticket #{position.ticket} đã được gửi thành công."
            )
            return result
        logger.error(
            f"❌ Đóng lệnh cho ticket #{position.ticket} thất bại. Retcode: {result.retcode if result else 'N/A'}, Error: {mt5.last_error()}"
        )
        return None
    ...
    def modify_position(self, ticket_id: int, sl_price: float, tp_price: float) -> bool:
        ...
        if result and result.retcode == mt5.TRADE_RETCODE_DONE:
            logger.info(
                f"Sửa lệnh #{ticket_id} thành công. SL mới: {sl_price:.5f}, TP mới: {tp_price:.5f}"
            )
            return True
        logger.error(
            f"Sửa lệnh #{ticket_id} thất bại. Retcode: {result.retcode if result else 'N/A'}, Error: {mt5.last_error()}"
        )
        return False
    ...
    def calculate_profit(
        ...
        if not self._is_connected:
            return None
        mt5_order_type = (
            mt5.ORDER_TYPE_BUY if order_type_str == "LONG" else mt5.ORDER_TYPE_SELL
        )
        profit = mt5.order_calc_profit(
            mt5_order_type, symbol, volume, entry_price, current_price
        )
        return profit
    ...
    def calculate_lot_size(
        ...
        try:
            ...
            if abs(entry_price - sl_price) < min_distance:
                logger.warning(
                    f"SL quá gần cho {symbol}. Khoảng cách hiện tại: {abs(entry_price - sl_price):.5f} < Yêu cầu: {min_distance:.5f}"
                )
                ...
                logger.info(
                    f"Tự động điều chỉnh SL cho {symbol} về mức an toàn: {sl_price:.5f}"
                )
            ...
            if loss_per_lot is None or loss_per_lot >= 0:
                logger.critical(
                    f"TÍNH TOÁN LỖ THẤT BẠI cho {symbol} ngay cả sau khi điều chỉnh SL. Giá trị loss_per_lot: {loss_per_lot}"
                )
                return min_vol, sl_price

            if abs(loss_per_lot) < 0.00001:
                logger.critical(
                    f"LỖI TOÁN HỌC: Mức lỗ ({loss_per_lot}) quá nhỏ (gần bằng 0). Trả về Min Lot để an toàn."
                )
                return min_vol, sl_price
            ...
            if lot_size < min_vol:
                ...
                if strict_min_lot:
                    logger.warning(
                        f"Lot size tính toán ({lot_size:.2f}) < mức tối thiểu ({min_vol}). STRICT_MIN_LOT = ON -> TỪ CHỐI LỆNH."
                    )
                    return 0.0, sl_price
                else:
                    logger.warning(
                        f"Lot size tính toán ({lot_size:.2f}) < mức tối thiểu ({min_vol}). Sử dụng mức tối thiểu."
                    )
                    return min_vol, sl_price
            if lot_size > max_vol:
                logger.warning(
                    f"Lot size tính toán ({lot_size:.2f}) > mức tối đa ({max_vol}). Sử dụng mức tối đa."
                )
                return max_vol, sl_price

            logger.debug(
                f"Tính toán lot size thành công cho {symbol}: {lot_size:.2f} (Rủi ro: ${risk_amount_usd:.2f})"
            )
    ...
    def validate_order_before_placement(
        ...
        try:
            ...
            if lot_size < symbol_info.volume_min:
                return (
                    False,
                    f"Lot size {lot_size} < tối thiểu {symbol_info.volume_min}",
                )
            ...
            if symbol_info.volume_step > 0:
                ...
                if abs(rounded_lot - lot_size) > 1e-5:
                    return (
                        False,
                        f"Lot size {lot_size} không đúng bước nhảy {symbol_info.volume_step}",
                    )
            ...
            if sl_price > 0:
                ...
                if order_type == mt5.ORDER_TYPE_BUY and sl_price >= entry_price:
                    return (
                        False,
                        f"SL sai hướng: BUY nhưng SL ({sl_price:.2f}) >= Entry ({entry_price:.2f}). Kiểm tra lại Swing Point.",
                    )
                if order_type == mt5.ORDER_TYPE_SELL and sl_price <= entry_price:
                    return (
                        False,
                        f"SL sai hướng: SELL nhưng SL ({sl_price:.2f}) <= Entry ({entry_price:.2f}). Kiểm tra lại Swing Point.",
                    )
                ...
                if min_distance_price > 0:
                    if (
                        ...
                        and entry_price - sl_price < min_distance_price
                    ):
                        return (
                            False,
                            f"SL quá gần. Khoảng cách {entry_price - sl_price:.5f} < yêu cầu {min_distance_price:.5f}",
                        )
                    if (
                        order_type == mt5.ORDER_TYPE_SELL
                        and sl_price - entry_price < min_distance_price
                    ):
                        return (
                            False,
                            f"SL quá gần. Khoảng cách {sl_price - entry_price:.5f} < yêu cầu {min_distance_price:.5f}",
                        )

            if tp_price > 0 and min_distance_price > 0:
                if (
                    order_type == mt5.ORDER_TYPE_BUY
                    and tp_price - entry_price < min_distance_price
                ):
                    return (
                        False,
                        f"TP quá gần. Khoảng cách {tp_price - entry_price:.5f} < yêu cầu {min_distance_price:.5f}",
                    )
                if (
                    order_type == mt5.ORDER_TYPE_SELL
                    and entry_price - tp_price < min_distance_price
                ):
                    return (
                        False,
                        f"TP quá gần. Khoảng cách {entry_price - tp_price:.5f} < yêu cầu {min_distance_price:.5f}",
                    )

            # --- [V4.3 NEW] KIỂM TRA FREE MARGIN (KÝ QUỸ DƯ) ---
            margin_required = mt5.order_calc_margin(
                order_type, symbol, lot_size, entry_price
            )
            ...
            if margin_required is not None and acc_info is not None:
                ...
                if margin_required > safe_margin_free:
                    return (
                        False,
                        f"Thiếu Margin (Cần: ${margin_required:.2f}, Free Margin: ${acc_info.margin_free:.2f})",
                    )
    ...
    def get_market_status(self, symbol: str) -> dict:
        ...
        try:
            ...
            if not symbol_info or not tick:
                return {
                    "status": "error",
                    "message": "Không lấy được dữ liệu thị trường",
                }
