...
class TradeManager:
    ...
    def execute_bot_trade(self, direction, symbol, context, market_mode="ANY", signal_class="ENTRY"):
        ...
        if result and result.retcode == 10009:
            ...
            bot_tactic = brain.get("risk_tsl", {}).get("bot_tsl", getattr(config, "BOT_DEFAULT_TSL", "BE+STEP_R+SWING"))
            ...
            if parent_pos and signal_class in ["DCA", "PCA"]:
                ...
                self.state["initial_r_dist"][s_child] = self.state["initial_r_dist"].get(s_parent, sl_distance)
                save_state(self.state)
                
                self.log(f"🔥 [{signal_class}] Mẹ #{s_parent} đẻ Con #{s_child} | Vol: {lot_size:.2f}")
                if signal_class == "DCA":
                    threading.Thread(target=self._adjust_basket_tp, args=(parent_pos.ticket,), daemon=True).start()
            else:
                self.log(f"🚀 [BOT EXEC] {direction} {symbol} #{ticket_id} | Lot: {lot_size:.2f} | TSL: {bot_tactic}")
    ...
    def _adjust_basket_tp(self, parent_ticket):
        ...
        positions = [p for p in self.connector.get_all_open_positions() if p.ticket in tickets]
    ...
    def execute_manual_trade(self, direction, preset_name, symbol, strict_mode, 
                             manual_lot=0.0, manual_tp=0.0, manual_sl=0.0, bypass_checklist=False, tactic_str="OFF"):
        config.SYMBOL = symbol 
        acc_info = self.connector.get_account_info()
        ...
        params = getattr(config, "PRESETS", {}).get(preset_name, {"SL_PERCENT": 0.4, "TP_RR_RATIO": 1.5, "RISK_PERCENT": 0.3})
        tick = mt5.symbol_info_tick(symbol)
        sym_info = mt5.symbol_info(symbol)
        if not tick or not sym_info: return "ERR_NO_TICK"
        ...
        if result and result.retcode == 10009:
            ...
            save_state(self.state)
            self.log(f"🚀 [USER EXEC] {direction} {symbol} #{result.order} | Vol: {lot_size:.2f} | TSL: {tactic_str}")
    ...
    def update_running_trades(self, account_type="STANDARD", all_market_contexts=None):
        ...
        try:
                ...
                for ticket in closed_tickets:
                    ...
                    if s_ticket in self.state.get("parent_baskets", {}):
                        ...
                        for child_t in child_tickets:
                            ...
                            if child_pos:
                                self.log(f"⚠️ [BASKET CLOSE] Đóng lệnh Con #{child_t} do Mẹ #{ticket} đã chốt!")
            ...
            for pos in tracked_positions:
                ...
                sym_ctx = all_market_contexts.get(pos.symbol, {}) if all_market_contexts else {}
