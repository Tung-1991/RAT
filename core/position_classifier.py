...
def is_grid_position(pos: Any, magics: Optional[Dict[str, int]] = None) -> bool:
    ...
    return (grid_magic is not None and _magic(pos) == int(grid_magic)) or GRID_COMMENT_PREFIX in _comment(pos)


def is_bot_position(pos: Any, magics: Optional[Dict[str, int]] = None) -> bool:
    if pos is None or is_grid_position(pos, magics):
        return False
...
def is_hedge_position(pos: Any, magics: Optional[Dict[str, int]] = None) -> bool:
    ...
    return (hedge_magic is not None and _magic(pos) == int(hedge_magic)) or HEDGE_COMMENT_PREFIX in _comment(pos)
